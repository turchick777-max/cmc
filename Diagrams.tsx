...
// --- BOT WORKFLOW VISUALIZER (FIXED FOR MOBILE) ---
export const BotWorkflowVisualizer: React.FC = () => {
  ...
  const steps = [
    { icon: Smartphone, label: "TELEGRAM", activeColor: "border-white text-white shadow-[0_0_30px_rgba(255,255,255,0.2)]" },
    { icon: Search, label: "СКАНЕР", activeColor: "border-blue-500 text-blue-500 shadow-[0_0_40px_rgba(59,130,246,0.5)]" },
    { icon: FileText, label: "ОТЧЕТ", activeColor: "border-green-500 text-green-500 shadow-[0_0_40px_rgba(34,197,94,0.5)]" }
  ];
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-6 sm:p-8 md:p-12">
      
      {/* Вертикальный layout для мобильных, горизонтальный для десктопа */}
      <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8 md:gap-16">
        
        {steps.map((stepData, index) => {
          const Icon = stepData.icon;
          const isActive = index === 0 ? step === 0 : index === 1 ? step === 1 : step >= 2;
          
          return (
            <React.Fragment key={index}>
              <div className={`flex flex-col items-center gap-3 sm:gap-6 transition-all duration-500 ${isActive ? 'opacity-100 scale-105 sm:scale-110' : 'opacity-40 blur-[1px]'}`}>
                <div className={`w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-2xl sm:rounded-[2rem] flex items-center justify-center bg-[#1d1d1f] border-2 transition-all duration-500 ${isActive ? stepData.activeColor : 'border-white/10 text-gray-500'}`}>
                  <Icon size={28} className="sm:w-8 sm:h-8 md:w-10 md:h-10" />
                </div>
                <span className="text-xs sm:text-sm md:text-base font-bold tracking-widest text-gray-300">{stepData.label}</span>
              </div>
              
              {/* Connector line - вертикальная на мобильных, горизонтальная на десктопе */}
              {index < steps.length - 1 && (
                <>
                  {/* Мобильная версия - вертикальная линия */}
                  <div className="sm:hidden w-[2px] h-8 bg-gradient-to-b from-transparent via-white/30 to-transparent"></div>
                  {/* Десктопная версия - горизонтальная линия */}
                  <div className="hidden sm:block w-8 md:w-16 h-[2px] bg-gradient-to-r from-transparent via-white/30 to-transparent"></div>
                </>
              )}
            </React.Fragment>
          );
      ...
      {/* Status message */}
      <div className="mt-10 sm:mt-16 md:mt-20 text-center min-h-[60px] sm:min-h-[48px] flex items-center justify-center">
        <motion.p 
          key={step}
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-white text-xl sm:text-2xl md:text-3xl font-semibold tracking-tight px-4"
        >
          {messages[step]}
        </motion.p>
      </div>
